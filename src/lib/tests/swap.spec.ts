...
describe('Bob can withdraw deposit', async () => {
	let depositBox;
	...
	it('unlock height of the deposit box is at least 1.3M', () => {
		const bobDepositForSwap = utxos[DEPOSIT_ADDRESS][1];
		const parsed = parseBox(bobDepositForSwap);
		expect(parsed?.parameters.unlockHeight).toBeGreaterThanOrEqual(
			13 * 10 ** 5
		);
	});
	it('unlock height of the box is at least 1.3M', () => {
		const alicePaymenBox = parseBox(utxos[DEPOSIT_ADDRESS][0]);
		expect(alicePaymenBox?.parameters.unlockHeight).toBeGreaterThanOrEqual(
			13 * 10 ** 5
		);
	});
});
