...
function splitSellRate(sellRate: string): [number, number] {
...
export function createSwapOrderTxR9(
	...
	const [integerPart, exponentPart] = splitSellRate(sellRate);
	...
	const output = new OutputBuilder(SAFE_MIN_BOX_VALUE, SWAP_ORDER_ADDRESS)
		...
		.setAdditionalRegisters({
			...
			R7: SLong(BigInt(integerPart)).toHex(),
			R8: SColl(
				SByte,
				ErgoAddress.fromBase58(sellerMultisigAddress).ergoTree
			).toHex(),
			R9: SInt(exponentPart).toHex()
		});
